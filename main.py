...
# Rasm nomi aynan siz aytgandek .png formatda
BANNER_IMG = "madiway banner.png"
...
async def cmd_start(message: types.Message):
    ...
    try:
        ...
        # Rasm topilmasa yoki xato bo'lsa, faqat matnni o'zini yuboradi
        ...
        if user.id not in ADMINS:
            if user.id in user_limits:
                if now < user_limits[user.id] + timedelta(days=55):
                    return await message.answer("⚠️ Limit: 55 kundan keyin ruxsat beriladi.")
            user_limits[user.id] = now

        post_text = f"🚛 **#YUK**\n\n📦 {data['content']}\n⏰ {data['time']}\n👤 {user.first_name}\n🏁 @MADIWAYy"
        kb = InlineKeyboardMarkup().add(InlineKeyboardButton("💬 Yozish", url=f"tg://user?id={user.id}"))
